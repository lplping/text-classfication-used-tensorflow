    ...
    def __init__(self, sequence_length, num_classes, vocab_size,batch_size,
      embedding_size, hidden_size, num_layers):
        ...
        def cell():
            ...
            gruCell = tf.contrib.rnn.GRUCell(hidden_size)
            #添加 dropout layer, 一般只设置 output_keep_prob
            gruCell = tf.contrib.rnn.DropoutWrapper(cell=gruCell, output_keep_prob=self.dropout_keep_prob)
            return gruCell
        ...
        with tf.name_scope("output"):
            ...
            # 分类器
            self.scores = tf.layers.dense(last, num_classes, name='fc2')
